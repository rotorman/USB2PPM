...
//
// Last modified:
// 2020-11-09 Risto Kõiva, Initial Version
// 2021-03-29 Risto Kõiva, Added Joystick input
...
namespace ServoControl
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private DispatcherTimer dtJoystickTimer = new DispatcherTimer(); // Fetches Joystick positions
        ...
        private void btConnectDisconnect_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (cbJoystick.IsChecked == true)
                    cbJoystick.IsChecked = false;
            }
        }
        ...
        private void cbJoystick_Checked(object sender, RoutedEventArgs e)
        {
            slider_ch1.Value = DEFAULTSERVOPOSITION;
            slider_ch2.Value = DEFAULTSERVOPOSITION;
            slider_ch3.Value = DEFAULTSERVOPOSITION;
            ...
            dtJoystickTimer.Tick += new EventHandler(dtJoystickTimer_Tick);   // Register new event for timer to fire
            dtJoystickTimer.Interval = new TimeSpan(0, 0, 0, 0, 20);          // Set timer interval to 20ms evaluating to 50 Hz frame update rate
            dtJoystickTimer.Start();
        }

        private void cbJoystick_Unchecked(object sender, RoutedEventArgs e)
        {
            dtJoystickTimer.Stop(); // Stop the AutoGen timer
        }

        // dtJoystickTimer_Tick event gets called when the dtJoystickTimer fires.
        private void dtJoystickTimer_Tick(object sender, EventArgs e)
        {
            JoystickState state = new JoystickState();

            // Gets data from first Joystick
            state = Sticks[0].GetCurrentState();

            slider_ch1.Value = state.X;
            slider_ch2.Value = state.Y;
            slider_ch3.Value = state.Z;
        }
            ...
            foreach (DeviceInstance device in Input.GetDevices(DeviceClass.GameController, DeviceEnumerationFlags.AttachedOnly))
            {
                try
                {
                    ...
                    foreach (DeviceObjectInstance deviceObject in stick.GetObjects())
                    {
                        ...
                        {
                            stick.GetObjectPropertiesById((int)deviceObject.ObjectType).SetRange(1000, 2000);
                        }
                    }
                    sticks.Add(stick);
                }
